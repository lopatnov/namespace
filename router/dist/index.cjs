...
async function handleNavigation(router) {
	...
	(0, _lopatnov_namespace.emit)(router.ns, "router:beforeNavigate", nav);
	if (nav.cancelled) {
	...
	if (!matched) {
		...
		(0, _lopatnov_namespace.emit)(router.ns, "router:notfound", pathOnly);
		return;
	}
	if (!router.container) return;
	(0, _lopatnov_namespace.emit)(router.ns, "router:before", pathOnly, matched.params);
	try {
		const cleanup = await matched.entry.handler(router.container, matched.params, query);
		if (typeof cleanup === "function") router.currentCleanup = cleanup;
		(0, _lopatnov_namespace.emit)(router.ns, "router:after", pathOnly, matched.params);
	} catch (err) {
		(0, _lopatnov_namespace.emit)(router.ns, "router:error", pathOnly, err);
	}
}
