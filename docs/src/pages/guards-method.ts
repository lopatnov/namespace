import { guardsMethods } from "../data/guards-methods.ts";
import template from "./guards-method.html";

export default function guardsMethod(container: Element, params: Record<string, string>) {
  const group = guardsMethods.find((g) => g.slug === params.method);
  ...
  if (!group) {
    container.innerHTML = `
      <div class="py-4">
        ...
        <a href="/guards" data-nav class="btn btn-outline-primary">Back to Guards</a>
      </div>`;
    return;
  }
