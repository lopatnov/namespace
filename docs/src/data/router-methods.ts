...
export const routerMethods: MethodGroup[] = [
  {
    slug: "createRouter",
    ...
    methods: [
      {
        ...
        description:
          "Create a router and register it in the namespace. Options: `mode` ('hash' | 'history', default 'hash'), `root` (CSS selector for container, default '#app').",
...
start(router);`,
      },
      {
        ...
        description:
          "Start the router — listen to URL changes, intercept `[data-nav]` link clicks, and render the current route.",
    ...
    methods: [
      {
        ...
        description:
          "Register a route. Patterns support `:param` placeholders and `*` wildcard. Handler receives `(container, params, query)` and may return a cleanup function.",
...
route(router, '*', (container) => {
  container.innerHTML = '<h1>404</h1>';
      ...
      },
      {
        ...
        description:
          "Navigate to a path programmatically. In hash mode sets `location.hash`, in history mode calls `pushState`.",
    ...
    methods: [
      {
        ...
        description:
          "Create a lazy-loaded route handler from a dynamic import. The module is fetched only on first navigation to the route, then cached. Perfect for code splitting.",
