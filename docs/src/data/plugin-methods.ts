...
export const pluginMethods: MethodGroup[] = [
  {
    slug: "definePlugin",
    ...
    methods: [
      {
        name: "definePlugin",
        signature:
          "definePlugin<TOptions>(plugin: NamespacePlugin<TOptions>): NamespacePlugin<TOptions>",
        description:
          "Identity function for TypeScript inference. Zero runtime overhead. Wrap your plugin object in `definePlugin` to get full type-checking on `install` / `uninstall` callbacks and options.",
  ...
  uninstall(ns) {
    // cleanup if needed
      ...
      },
      {
        name: "usePlugin",
        signature:
          "usePlugin<TOptions>(ns: Namespace, plugin: NamespacePlugin<TOptions>, options?: TOptions): void",
        description:
          "Install a plugin on a namespace. Idempotent — calling multiple times with the same plugin has no effect after the first install. Emits `plugin:installed` event on success. Accepts an optional options argument when `TOptions` is not `void`.",
    ...
    methods: [
      {
        ...
        description:
          "Check whether a plugin is currently installed on a namespace. Accepts either a plugin object or a plugin `id` string.",
...
if (installed(app.ns, 'log')) {
  ...
  logger.info('Hello');
      ...
      },
      {
        ...
        description:
          "Uninstall a plugin from a namespace. Calls `plugin.uninstall(ns)` if defined, then emits `plugin:uninstalled`. Accepts a plugin object or an id string. Safe to call if the plugin is not installed.",
