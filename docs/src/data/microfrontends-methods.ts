...
export const microfrontendsMethods: MethodGroup[] = [
  {
    slug: "createBus",
    title: "createBus",
    methods: [
      {
        ...
        description:
          "Create a cross-tab event bus backed by BroadcastChannel. Messages are delivered to all subscribers in the current tab AND to all other tabs/windows/workers sharing the same channel name. Local delivery is synchronous. Falls back to a local-only bus when BroadcastChannel is not available. Call `bus.destroy()` to close the channel.",
