...
export const guardsMethods: MethodGroup[] = [
  {
    slug: "guard",
    title: "guard",
    methods: [
      {
        ...
        description:
          "Register a synchronous route guard. The guard function is called before every navigation to a path matching `pattern`. Return `true` to allow, `false` to cancel (stays on current page), or a path string to redirect. Patterns support `:param` and `*` wildcard (same syntax as `route()`). Returns an unsubscribe function.",
    ...
    methods: [
      {
        ...
        description:
          "Register an access check for a namespace key. Multiple protections on the same key are AND-combined — all must return `true` for access to be granted. Returns an unsubscribe function.",
...
unsub();`,
      },
      {
        ...
        description:
          "Check whether access to a protected namespace key is currently allowed. Returns `true` if no protections are registered, or if all registered check functions return `true`.",
