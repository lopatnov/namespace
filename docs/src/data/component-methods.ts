...
export const componentMethods: MethodGroup[] = [
  {
    ...
    methods: [
      {
        ...
        description:
          "Define a reusable component. Accepts a `name`, `template` HTML string, optional `styles` CSS, a `state` factory, `methods`, and lifecycle hooks `mounted` / `unmounted`. Returns the same options object typed as a `ComponentDef` — pass it directly to `mount()`.",
    ...
    methods: [
      {
        name: "mount",
        signature:
          "mount<P, S>(def: ComponentDef<P, S>, container: Element | string, props?: P): ComponentInstance",
        description:
          "Mount a component into a container. The `container` can be a CSS selector string or an existing `Element`. Props are passed to the `state` factory. Returns a `ComponentInstance` with the root `el` element and a `destroy()` method to unmount. Scoped styles are injected into `<head>` once per component name.",
