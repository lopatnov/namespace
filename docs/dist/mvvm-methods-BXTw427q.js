//#region src/data/mvvm-methods.ts
const mvvmMethods = [
	{
		slug: "reactive",
		title: "reactive",
		methods: [{
			...
			description: "Wrap a plain object in a reactive Proxy. Property reads inside active effects are tracked as dependencies; assignments notify all dependents. Nested objects are recursively proxied and cached — the same Proxy is always returned for the same underlying object. Note: in-place array mutations (push, pop, splice…) do not auto-trigger effects. Use immutable style: `state.items = [...state.items, newItem]`.",
...
state.items = [...state.items, 'cherry'];`
	...
	},
	{
		...
		methods: [{
			...
			description: "Activate `data-bind` bindings on all elements within a DOM subtree. Scans the subtree for `[data-bind]` attributes and sets up reactive DOM updates. Returns a cleanup function that removes all active effects and event listeners. The `root` argument can be a CSS selector string or an Element.",
...
stop();`
	...
	},
	{
		...
		methods: [{
			...
			description: "Set the element's `textContent` to the evaluated expression. Special HTML characters are escaped automatically. Re-runs whenever reactive dependencies change.",
			example: `<!-- HTML -->
...
state.count = 5;            // → <p>5 items</p>`
			...
			description: "Set the element's `innerHTML` to the evaluated expression. Use when you need to render rich HTML content. Caution: only bind to trusted content to avoid XSS.",
			example: `<!-- HTML -->
...
state.content = '<strong>Bold</strong> text';`
	...
	},
	{
		...
		methods: [{
			...
			description: "Show or hide the element by toggling `style.display`. The element remains in the DOM — its child bindings stay active. Use `visible` when the element is frequently toggled.",
			example: `<!-- HTML -->
...
state.isLoggedIn = false;  // div hidden`
			...
			description: "Conditionally show or hide an element. In this implementation, `if` behaves like `visible` (display toggle). The element and its bindings remain active regardless.",
			example: `<!-- HTML -->
  ...
  <h2 data-bind="text: user.name"></h2>
	...
	},
	{
		...
		methods: [{
			...
			description: "Two-way binding for text inputs, textareas, and selects. Reads from state when state changes and writes back to state on `input`/`change` events. The expression must be an assignable path (e.g. `searchQuery`, `user.name`).",
			example: `<!-- HTML -->
...
// state.searchQuery === 'hello'`
			...
			description: "Two-way binding for checkboxes. Reads the boolean state into `element.checked` and writes back on `change` events.",
			example: `<!-- HTML -->
...
// User checks the box → state.acceptTerms === true`
	...
	},
	{
		...
		methods: [{
			...
			description: "Toggle CSS classes based on boolean expressions. The binding object maps class names to expressions. Multiple classes can be controlled in one binding.",
			example: `<!-- HTML -->
...
state.isEnabled = false;   // adds class 'disabled'`
			...
			description: "Set element attributes from expressions. Pass `null`, `false`, or `undefined` to remove the attribute. Multiple attributes can be bound in one object.",
			example: `<!-- HTML -->
...
state.user.name = 'Alice';`
	...
	},
	{
		...
		methods: [{
			...
			description: "Attach event listeners. Handlers are evaluated once at bind time and are assumed to be stable references. Multiple events can be bound in one object.",
			example: `<!-- HTML -->
