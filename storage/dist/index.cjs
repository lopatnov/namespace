...
function persist(ns, options) {
	...
	function save(changedKey) {
		...
		const doSave = () => {
			...
			if (value === void 0) storage.removeItem(storageKey);
				...
				let serialized;
				try {
					serialized = JSON.stringify((0, _lopatnov_namespace.toJSON)(value));
				} catch {
	...
	const unsubChange = (0, _lopatnov_namespace.on)(ns, "change", (key) => save(key));
	const unsubDelete = (0, _lopatnov_namespace.on)(ns, "delete", (key) => save(key));
	return () => {
		unsubChange();
		unsubDelete();
